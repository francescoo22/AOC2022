...
public:
    ...
    pair <int, int> get_position (int round, int height, int width){ // da 1 a height
...
int main (){
    ...
    int round = q.front().round + 1;
    q.clear();
    auto temp = start;
    start = {end.first + 1, end.second};
    end = {temp.first + 1, temp.second};

    q.push_back(Position(start.first, start.second, round));
    ...
    while (q.front().x != end.first || q.front().y != end.second){
        ...
        q.pop_front();
        bool move_up, move_down, move_left, move_right, stay = true;
        if (input[p.x - 1][p.y] == '#') move_up = false; else move_up = true;
        if (p.x >= HEIGHT + 1 || input[p.x + 1][p.y] == '#') move_down = false; else move_down = true;
        if (input[p.x][p.y - 1] == '#') move_left = false; else move_left = true;
        if (input[p.x][p.y + 1] == '#') move_right = false; else move_right = true;
